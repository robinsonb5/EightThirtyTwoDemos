...
unsigned int FileOpen(fileTYPE *file, const char *name)
{
	...
	if(res=FileFind(name))
	{
		pEntry=res->de;
                ...
                file->size = SwapBBBB(pEntry->FileSize);
                file->start_cluster = SwapBB(pEntry->StartCluster) + (fat32 ? (SwapBB(pEntry->HighCluster) & 0x0FFF) << 16 : 0); 
