...
unsigned int current_directory_start=0;
...
void SwapPartitionBytes(int i)
{
	...
	partitions[i].startlba=ConvBBBB_LE(partitions[i].startlba);
...
unsigned int FindDrive(void)
{
    ...
    printf("partition type: 0x%x (", sector_buffer[450]);
    fat32=0;
    switch (fattype)
    {
		case 0:
...
//        return(0);
    ...
    {
        printf("Near jump checked failed\n");
        return(0);
    }
    ...
    printf("cluster_mask: %x\r", cluster_mask);
	ChangeDirectory(0);
...
{
	struct FileFind_Result *res;
    DIRENTRY      *pEntry = NULL;        // pointer to current entry in sector buffer
	if(res=FileFind(name))
	{
		pEntry=res->de;
                strncpy(file->name, (const char*)pEntry->Name, sizeof(file->name));
                file->attributes = pEntry->Attributes;
                file->size = ConvBBBB_LE(pEntry->FileSize);
                file->start_cluster = ConvBB_LE(pEntry->StartCluster) + (fat32 ? (ConvBB_LE(pEntry->HighCluster) & 0x0FFF) << 16 : 0); 
                file->cluster =  file->start_cluster;
                file->sector = 0;
                file->entry.sector = res->et.sector;
                file->entry.index = res->et.index;
        ...
        for (iEntry = 0; iEntry < nEntries; iEntry++)
        {
            if ((iEntry & 0xF) == 0) // first entry in sector, load the sector
            {
				...
				for (i = 0; i < 16; i++) 
				{
					...
					{
						pEntry->StartCluster = ConvBB_LE(pEntry->StartCluster);
						pEntry->HighCluster = ConvBB_LE(pEntry->HighCluster);
						pEntry->FileSize = ConvBBBB_LE(pEntry->FileSize);
					}
					pEntry++;
				}		
                pEntry = (DIRENTRY*)sector_buffer;
...
//    return(fat32 ? fat_buffer.fat32[buffer_index] & 0x0FFFFFFF : fat_buffer.fat16[buffer_index]); // get FAT link
    return(fat32 ? ConvBBBB_LE(fat_buffer.fat32[buffer_index]) & 0x0FFFFFFF : ConvBB_LE(fat_buffer.fat16[buffer_index])); // get FAT link for 68000
}
...
//        file->cluster = fat32 ? fat_buffer.fat32[i] & 0x0FFFFFFF: fat_buffer.fat16[i]; // get FAT link
        file->cluster = fat32 ? ConvBBBB_LE(fat_buffer.fat32[i]) & 0x0FFFFFFF : ConvBB_LE(fat_buffer.fat16[i]); // get FAT link for 68000 
    }
...
//        file->cluster = fat32 ? ConvBBBB_LE(fat_buffer.fat32[i]) & 0x0FFFFFFF : ConvBB_LE(fat_buffer.fat16[i]); // get FAT-link for 68000
        if (fat32)
        {
            file->cluster = ConvBBBB_LE(fat_buffer.fat32[i]) & 0x0FFFFFFF; // get FAT32 link
            if (file->cluster == 0x0FFFFFFF) // FAT32 EOC
                return 0;
        }
        else
        {
            file->cluster = ConvBB_LE(fat_buffer.fat16[i]); // get FAT16 link
...
//                                fat_buffer.fat32[buffer_index] = 0x0FFFFFFF; // FAT32 EOC change
                            ...
                            pEntry->CreateDate = ConvBB_LE(FILEDATE(2009, 9, 1));
                            pEntry->CreateTime = ConvBB_LE(FILETIME(0, 0, 0));
                            pEntry->AccessDate = ConvBB_LE(FILEDATE(2009, 9, 1));
                            pEntry->ModifyDate = ConvBB_LE(FILEDATE(2009, 9, 1));
                            pEntry->ModifyTime = ConvBB_LE(FILETIME(0, 0, 0));
...
//                            pEntry->FileSize = file->size;
                            pEntry->StartCluster = (unsigned short)ConvBB_LE(cluster); // for 68000
                            pEntry->HighCluster = fat32 ? (unsigned short)ConvBB_LE(cluster >> 16) : 0; // for 68000
...
unsigned int UpdateEntry(fileTYPE *file)
{
    ...
    if ((ConvBBBB_LE(pEntry->FileSize) + cluster_size - 1) / (cluster_size << 9) != (file->size + cluster_size - 1) / (cluster_size << 9))
    {
        ...
        printf("pEntry->FileSize = %d\r", ConvBBBB_LE(pEntry->FileSize));
